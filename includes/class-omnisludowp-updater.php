...
class OMNISLUDOWP_Updater {
	...
	function pre_set_site_transient_update_themes( $transient ) {
		...
		if (	isset($transient->checked[ $this->theme_basename ] ) 
			...
			$update = $this->omnisludowp_check_for_updates( $this->theme_basename, $transient->checked[ $this->theme_basename ] );
		...
		if (is_wp_error( $update )){
			...
			add_action( 'admin_notices', array( $this, 'error_actualizacion_notice' ) );
	...
	function omnisludowp_check_for_updates( $theme_basename, $current_version ){

		// Recupero la información del repositorio,
		// en concreto la última release
		$repository_info = $this->github_repository_info();		

		if ( is_wp_error( $repository_info ) ) {
			$theme = $repository_info;
	...
	public function github_repository_info() {		
      ...
      error_log( "Error wp_remote_get => " . print_r( $response, true ) );
			return new WP_Error( 'github_repository_info_error', 'Error al recuperar información del respositorio');
		}
		// Decode the response.
		...
		if ( !isset($response['zipball_url'])){			
			...
			return new WP_Error( 'github_repository_info_error', 'Error no hay url de descarga');
		}
	...
	public function upgrader_source_selection( $source, $remote_source, $upgrader, $hook_extra = null ) {
		...
		if (	!(	 isset( $hook_extra['theme'] ) 
					&& ( $hook_extra['theme'] == $this->theme_basename) )){
	...
	/**
	* Mensaje de aviso error actuazlacion
	*/
	public function error_actualizacion_notice() {
		?>
		<div class="notice notice-error"><p>Error al comprobar actualizaciones disponibles para el tema. Compruebe las credenciales.</p></div>
		<?php
	}
